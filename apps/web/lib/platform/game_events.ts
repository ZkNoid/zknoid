...
export const getEventType = (event: ZkNoidEvent): ZkNoidEventType => {
    ...
    if (event.eventStarts > now) 
        return ZkNoidEventType.UPCOMING_EVENTS;

    if (event.eventEnds > now) 
        return ZkNoidEventType.CURRENT_EVENTS;
...
export const useEventTimer = (
    ...
    const [startsIn, setStartsIn] = useState<DurationObjectUnits>(getEventTime(event));
