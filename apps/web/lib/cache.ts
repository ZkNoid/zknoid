...
export async function fetchCache(cacheInfo: ContractCache): Promise<FetchedCache> {
    const fetchedCacheObject =
        await Promise.all(cacheInfo.files.map((file) => {
            return Promise.all([
                fetch(`${cacheInfo.cachePath}/${file}.header`).then(res => res.text()),
...
export const WebFileSystem = (fetchedCache: FetchedCache): any => ({
    read({ persistentId, uniqueId, dataType }: any) {
        ...
        if (currentId !== uniqueId) {
            console.log('current id did not match persistent id', persistentId, currentId, uniqueId);
        ...
        if (dataType === 'string') {
            ...
            return new TextEncoder().encode(fetchedCache.fetchedCacheInfo.get(persistentId)!.data);
        }
