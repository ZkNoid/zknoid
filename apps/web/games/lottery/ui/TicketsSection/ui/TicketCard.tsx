...
export default function TicketCard({
  ...
  const submitTicket = () => {
    setFinalized(true);
    addTicket({ numbers: [...symbols].map((x) => Number(x)), amount: amount });
  };

  return (
    <motion.div
      className={cn(
        'relative -mt-[23%] w-[24vw] rounded-[1.33vw] bg-right-accent p-[0.33vw] first:mt-0',
          ...
          <div className="ml-auto flex gap-[0.33vw] text-[1.07vw] text-right-accent">
            {amount == 0 ? (
              ...
              <div className={'flex flex-row gap-[0.26vw]'}>
                {ticketsAmount > 1 && (
                  <TicketBlockButton onClick={removeTicket}>
                    <svg
                      width="0.885vw"
                      height="0.885vw"
                      viewBox="0 0 17 18"
                ...
                <TicketsNumPicker amount={amount} setAmount={setAmount} />
              </div>
            )}
          </div>
        </div>
