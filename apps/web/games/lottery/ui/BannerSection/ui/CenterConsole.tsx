...
export default function CenterConsole({
  ...
  const leaderboard = roundInfo?.tickets
    .filter((ticket) => ticket.funds !== undefined)
    .sort((a, b) => (b.funds! > a.funds! ? 1 : -1))
    .slice(0, 3);

  return (
    <div className="absolute bottom-0 left-0 right-0 top-0 mx-auto h-full w-[19.4vw]">
      <div
        className={cn('mt-[1vw] flex w-[17.969vw] flex-col', {
      ...
      >
        <div
          className={
            'w-full text-center font-museo text-[1.719vw] font-bold uppercase'
          }
        >
          {roundToShow != lotteryStore.lotteryRoundId
        ...
        <div
          className={cn('flex flex-row gap-[0.781vw]', {
            'mt-[0.938vw]':
              roundToShow == lotteryStore.lotteryRoundId ||
              (roundToShow != lotteryStore.lotteryRoundId &&
                !roundInfo?.winningCombination),
        ...
        >
          {roundToShow == lotteryStore.lotteryRoundId ? (
            ...
            <div
              className={
                'flex w-full flex-col items-center justify-center gap-[0.26vw]'
              }
            >
