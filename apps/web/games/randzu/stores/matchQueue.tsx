...
export interface MatchQueueState {
  ...
  loadMatchQueue: (
    client: ClientAppChain<typeof randzuConfig.runtimeModules>,
    blockHeight: number
  ) => Promise<void>;
  loadActiveGame: (
    client: ClientAppChain<typeof randzuConfig.runtimeModules>,
...
>(
  immer((set) => ({
    ...
    async loadMatchQueue(
      ...
      const queueLength =
        await client.query.runtime.RandzuLogic.queueLength.get(
    ...
    async loadActiveGame(
      ...
      const activeGameId =
        await client.query.runtime.RandzuLogic.activeGameId.get(address);
      console.log('Active game id', activeGameId);
      const inQueue =
        await client.query.runtime.RandzuLogic.queueRegisteredRoundUsers.get(
          // @ts-expect-error
          new RoundIdxUser({
      ...
      if (
        ...
        const gameInfo = (await client.query.runtime.RandzuLogic.games.get(
      ...
      if (activeGameId?.greaterThan(UInt64.from(0)).toBoolean()) {
        const gameInfo =
          (await client.query.runtime.RandzuLogic.games.get(activeGameId))!;
